...
const SignupPage = () => {
    ...
    const handleGoogle = async () => {
        ...
        try { 
            ...
            // Control will navigate away
        } catch (err) { 
            console.error('Google signup error details:', err.code, err.message);
            if (err.code === 'auth/unauthorized-domain') {
                setError('This domain is not authorized for Google Sign-In. Check your Firebase Console → Authorized domains.');
            } else {
                setError(err.message.replace('Firebase: ', '').replace(/\(.*\)$/, '').trim());
            }
        }
    ...
    return (
        <div className="page-container" style={{
            ...
            <div style={{
                position: 'absolute', bottom: '20%', left: '10%', width: 350, height: 350, borderRadius: '50%',
                background: 'radial-gradient(circle, rgba(0,180,216,0.1) 0%, transparent 70%)', pointerEvents: 'none',
            }} />
            ...
            <div className="vc-card" style={{ width: '100%', maxWidth: 440, padding: '40px 36px', position: 'relative', zIndex: 10, margin: 'auto' }}>
                <div style={{ textAlign: 'center', marginBottom: 32 }}>
                    ...
                    <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>Join Hyderabad's smartest charging network</p>
                </div>
                ...
                <button onClick={handleGoogle}
                    style={{
                        width: '100%', padding: '12px', borderRadius: 12, border: '1.5px solid var(--bg-border)',
                        background: 'transparent', color: 'var(--text-primary)', cursor: 'pointer', fontSize: 15, fontWeight: 500,
                        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 20, transition: 'all 0.2s ease',
                    }}
                ...
                <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    <div>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <input type="text" value={name} onChange={e => setName(e.target.value)} className="vc-input" placeholder="Mohammed Meraj" style={{ paddingLeft: 40 }} />
                        </div>
                    </div>
                    <div>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="vc-input" placeholder="you@example.com" style={{ paddingLeft: 40 }} />
                        </div>
                    </div>
                    <div>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <input type={showPass ? 'text' : 'password'} required value={password} onChange={e => setPassword(e.target.value)} className="vc-input" placeholder="Min. 6 characters" style={{ paddingLeft: 40, paddingRight: 44 }} />
