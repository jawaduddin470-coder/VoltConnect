...
class _DriverProfileScreenState extends State<DriverProfileScreen> {
  ...
  void initState() {
    super.initState();
    _nameCtrl = TextEditingController(text: _auth.currentUser?.displayName ?? '');
  }
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      await _firestore.collection('users').doc(user.uid).update({'name': _nameCtrl.text});
  ...
  Future<void> _signOut() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('Sign Out', style: TextStyle(color: isDark ? Colors.white : AppColors.textPrimaryLight)),
        content: Text('Are you sure you want to sign out?', style: TextStyle(color: isDark ? AppColors.textSecondaryDark : AppColors.textSecondaryLight)),
        actions: [
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => setState(() {
              if (_isEditing) _nameCtrl.text = user.displayName ?? '';
          ...
          children: [
                ...
                GestureDetector(
                  ...
                  child: CircleAvatar(
                    ...
                    child: user.photoURL == null
                        ? Text(user.displayName?.isNotEmpty == true ? user.displayName![0].toUpperCase() : 'U', style: const TextStyle(color: AppColors.teal, fontSize: 32, fontWeight: FontWeight.bold))
                        : null,
                  ),
                ),
                if (_isEditing)
            ...
            if (_isEditing)
              TextField(
                ...
                style: TextStyle(color: textPri, fontSize: 20, fontFamily: 'Space Grotesk', fontWeight: FontWeight.bold),
                textAlign: TextAlign.center,
                decoration: const InputDecoration(
                  border: UnderlineInputBorder(borderSide: BorderSide(color: AppColors.teal)),
                  enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: AppColors.teal)),
                ),
              )
            else
              Text(user.displayName ?? 'Driver', style: TextStyle(color: textPri, fontSize: 20, fontFamily: 'Space Grotesk', fontWeight: FontWeight.bold)),
            ...
            if (_isEditing)
              Padding(
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(backgroundColor: AppColors.teal, foregroundColor: AppColors.bgDark, minimumSize: const Size(200, 40)),
            ...
            StreamBuilder<DocumentSnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                String plan = 'Free';
                Color color = textSec;
                String validText = 'No active plan';
                
                if (membership != null && membership['active'] == true) {
                  plan = membership['plan'] ?? 'Basic';
                  if (plan.contains('Pro')) color = AppColors.teal;
                  if (plan.contains('Premium')) color = AppColors.purple;
                  
                  if (membership['endDate'] != null) {
                    final endDate = DateTime.tryParse(membership['endDate']);
                    if (endDate != null) {
                      validText = 'Valid until: ${DateFormat('dd MMM yyyy').format(endDate)}';
                    }
                  }
                }
                ...
                return Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('VoltConnect Membership', style: TextStyle(color: textPri, fontSize: 16, fontWeight: FontWeight.w600)),
                      const SizedBox(height: 4),
                      Text(validText, style: TextStyle(color: textSec, fontSize: 12)),
                      const SizedBox(height: 16),
                      SizedBox(
                        width: double.infinity,
                        child: OutlinedButton(
                          style: OutlinedButton.styleFrom(foregroundColor: textPri, side: BorderSide(color: borderCol)),
            ...
            Container(
              ...
              child: Row(
                children: [
                  _buildStat('Sessions', '12', null, isDark, textPri, textSec),
                  Container(width: 1, height: 40, color: borderCol),
                  _buildStat('Charged', '240 kWh', null, isDark, textPri, textSec),
                  Container(width: 1, height: 40, color: borderCol),
                  _buildStat('CO₂ Saved', '196 kg', AppColors.success, isDark, textPri, textSec),
                ],
              ),
            ),
